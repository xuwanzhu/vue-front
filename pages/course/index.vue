              ...
              <li v-for="item in data.items" :key="item.id">
                <div class="cc-l-wrap">
                  ...
                  <section class="course-img">
                    ...
                    <div id="picture">
                      <img :src="item.cover" width="270" height="150" class="img-responsive" alt="图片已失效">
                    </div>
                  ...
                  <section class="mt10 hLh20 of">
                    ...
                    <span class="fl jgAttr c-ccc f-fA">
                      <!--TODO <i class="c-999 f-fA">{{ item.viewCount }}人浏览</i> -->
                      <i class="c-999 f-fA">{{item.buyCount}}人购买</i>
                      |
                      <i class="c-999 f-fA">共{{ item.lessonNum}}课时</i>
                    </span>
...
export default {
  ...
  data () {
    return {
      ...
      //下面为选中效果
      oneIndex: -1, //一级分类选中效果,不是随便传的，用来表示显示哪个样式
      twoIndex: -1, //二级分类选中效果
  ...
  watch: {
    $route (to, from) {
      console.log('课程路由变化......')
      this.searchObj.title = this.$route.query.courseName //重新给搜索title赋值
      this.initCourse()
    }
  },
  ...
  //加载完渲染时
  created () {
    this.searchObj.title = this.$route.query.courseName  //搜索课程的名称
    ...
    //数据回显
    if (this.$route.query.subjectLevelOneId) {
      this.show(this.$route.query.subjectLevelOneId)
    }
  ...
  methods: {
    ...
    //0、样式回显，即显示课程列表页的样式
    show (subjectLevelOneId) {
      course.getSubjectIndex(subjectLevelOneId)
        .then(response => {
          //1获取index
          let index = response.data.data.index
          //2调用方法作数据回显
          this.searchOne(subjectLevelOneId, index) //index不是随便传的，用来显示选中样式
        })
    ...
    //3、点击一级分类，显示对应的二级分类，查询数据
    searchOne (subjectParentId, index) {
      this.oneIndex = index //显示选中样式
