        ...
        <div class="fr" id="AgreeDiv">
          <label for="Agree">
            <p class="on"><input type="checkbox" checked="checked">我已阅读并同意<a href="javascript:"
                target="_blank">《在线教育平台购买协议》</a></p>
          </label>
        </div>
        <div class="clear"></div>
        <div class="Main fl">
          <div class="fl">
            <a :href="'/course/'+order.courseId" id="pic">返回课程详情页</a>
          </div>
          <div class="fr">
            <p>共 <strong class="red">1</strong> 件商品，合计<span class="red
f20">￥<strong id="AllPrice">{{order.totalFee}}</strong></span></p>
          </div>
        </div>
...
export default {
  ...
  asyncData ({ params, error }) {
    return orderApi.getOrderById(params.oid).then(response => {
      return {
        order: response.data.data.item  //订单页封装对象
      }
    })
  },
  ...
  methods: {
    //2点击去支付，跳转到支付页面
    toPay () {
      this.$router.push({ path: '/pay/' + this.order.orderNo }) //this.order.orderNo :订单号
    }
  }
